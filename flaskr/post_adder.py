...
class PostAdder:
    ...
    def add_post(
        ...
        if not quiet:
            print('Adding post from directory "{}"'.format(self.post_path))
    
        # Determine absolute paths to the Markdown and metadata files
        self.md_path = post_dir / 'post.md'
        self.meta_path = post_dir / 'post-meta.json'

        # Determine where the post data will live on this site's staticpath
        self.post_static_path = static_path / slug

        # Render Markdown file, getting the HTML and images sourced
        self.post_html, post_img_paths = \
            util.render_md_file(str(self.post_path), str(self.post_static_path))

        # Load post images into memory
        self.post_images = util.process_post_images(post_img_paths)

        # Read the metadata file
        self.post_json = util.read_meta_file(self.meta_path)

        # Process metadata
        self.post_data = util.process_post_meta(self.post_json)

        # Add post to database
        util.add_post_to_database(self.post_meta)
