...
def test_create_non_empty(client: FlaskClient):
    ...
    response = util.create_post(client, DEFAULT_USER, 'new-post', 'A New Post', 'Some random byline')
    assert response.status == '201 CREATED'
    assert util.is_post_json_valid(response.json)
    assert util.are_defaults_set(response.json)
...
def test_create_duplicate_slug(client: FlaskClient):
    ...
    assert util.create_post(client, DEFAULT_USER, slug='test_slug').status == '400 BAD REQUEST'
...
def test_create_invalid_fileid(client: FlaskClient):
    ...
    assert util.create_post(client, DEFAULT_USER, featured_id='1234').status == '400 BAD REQUEST'
    assert util.create_post(client, DEFAULT_USER, banner_id='1234').status == '400 BAD REQUEST'
    assert util.create_post(client, DEFAULT_USER, thumbnail_id='1234').status == '400 BAD REQUEST'


def test_get_all(client: FlaskClient):
    """The get() endpoint should return the three posts that were created."""
    res1 = util.create_post(client, DEFAULT_USER, slug='post-1', title='Post #1', byline='The first post')
    res2 = util.create_post(client, DEFAULT_USER, slug='post-2', title='Post #2', byline='The second post')
    res3 = util.create_post(client, DEFAULT_USER, slug='post-3', title='Post #3', byline='The third post')
...
def test_improper_pagination(client: FlaskClient):
    ...
    for i in range(10):
        util.create_post(client, DEFAULT_USER, slug=f'slug-{i}')
    assert util.get_posts(client, DEFAULT_USER, limit=5, offset=10).status == '404 NOT FOUND'
...
def test_get_single(client: FlaskClient):
    ...
    res_create = util.create_post(client, DEFAULT_USER, slug='post-1', title='Post #1', byline='The first post')
...
def test_add_tag_nonexistent(client: FlaskClient):
    ...
    res_create = util.create_post(client, DEFAULT_USER)
    post_id = res_create.json['id']
    assert util.add_tag_to_post(client, DEFAULT_USER, post_id, 'test').status == '400 BAD REQUEST'
